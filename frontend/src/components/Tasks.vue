<template>
  <div id="wrapper" class="container">
    <div class="back">
      <router-link :to="'/'+encodeURIComponent(account)" class="back-button">
        <i class="fa fa-chevron-left square-icon"></i> Zurück zu meinen Kursen
      </router-link>
      <div class="course">
        <span>
          <!--aktueller Kurs:-->
      ...
      <div class="container settings" :class="{open: settingsOpen}">
        ...
        <div class="row" style="margin-bottom: 0.5rem !important">
          <div style="display: flex; align-items: center;">
            ...
            <input
              v-if="settings.minimumType == 'points'"
              type="number"
              style="max-width: 65px; margin-left: 2px; display: inline;"
              step="1"
              v-model="settings.minimumPoints"
            >
            <input
              v-if="settings.minimumType == 'percent'"
              type="number"
              style="max-width: 65px; margin-left: 2px; display: inline;"
              min="0"
              max="100"
              step="0.1"
              v-model="settings.minimumPercent"
            >
        ...
        <div class="row">
          <div style="display: flex; align-items: center;">
            ...
            <input
              ...
              min="0"
              max="10"
              v-model="settings.minimumPresentations"
            >
          </div>
        </div>
        <div class="row" style="margin-top: 0.5rem !important; justify-content: space-between;">
          <button
            class="button"
            style="background-color: red; border-color: black;"
            @click="deleteCourse()"
          >{{del}}</button>
          <button class="button" style @click="saveSettings()">Speichern</button>
        </div>
      </div>
    </div>
    <table class="tasks">
      <thead>
        <tr>
          <th v-for="key in gridColumns">{{ key }}</th>
          <th class="small"></th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(entry, index) in gridData">
          ...
          <td style="text-align: center">{{ entry['presentations'] }}</td>
          <td class="small">
            <i
              class="fas fa-edit clickable-icon"
              @click="edit(entry)"
              style="margin-right: 5px; display: inline"
            ></i>
            <i
              class="fas fa-trash-alt clickable-icon"
              @click="deleteItem(index, entry['id'])"
        ...
        <tr>
          ...
          <td style="text-align: center">
            <b>{{ gesPres }}</b>
            <span v-if="minimumPresentations > 0">
              <span>/</span>
              <b style="opacity:0.7; margin-left:3px;">{{ minimumPresentations }}</b>
            </span>
          </td>
          <td>
            <span v-if="minimumType != 'none'">
              <strong style="margin-right:3px;">{{ gesPerc }}</strong>
              <span>/</span>
              <strong
                style="opacity:0.7; margin-left:3px;"
              >{{ minimumType == 'percent' ? minimumPercent + "%" : minimumPoints }}</strong>
            </span>
          </td>
        </tr>
      </tbody>
    ...
    <fieldset class="row">
      <div class="column full" style="flex-grow: 2">
        ...
        <input
          ...
          v-on:keyup.enter="setFocus('punkte')"
          style="color:#444;"
        >
      </div>
      <div class="column column-17">
        ...
        <input
          id="punkte"
          type="number"
          v-model.number="editData.points"
          v-on:keyup.enter="setFocus('maxPoints')"
          style="color:#444;"
        >
      </div>
      <div class="column column-17">
        ...
        <input
          id="maxPoints"
          type="number"
          v-model.number="editData.maxPoints"
      ...
      <div class="column column-17 full">
        ...
        <input
          id="pres"
          type="number"
          v-model.number="editData.presentations"
          v-on:keyup.enter="enter()"
          style="color:#444;"
        >
      </div>
      <div class="column full" style="flex: 0 0 11rem">
        <label>&nbsp;</label>
        <button
          class="button submit-button aniInput"
          type="button"
          @click="addTask()"
          v-if="isNew"
        >Hinzufügen</button>
...
export default {
  ...
  created() {
    ...
    api
      .GET("/perc/" + encodeURI(this.account) + "/" + encodeURI(this.course))
      .then(res => {
        this.minimumType = res.body.type || "none";
        this.minimumPoints =
          this.minimumType == "points" ? res.body.minimum : 50;
        this.minimumPercent =
          this.minimumType == "percent" ? res.body.minimum : 50;
      });
    api
      .GET("/pres/" + encodeURI(this.account) + "/" + encodeURI(this.course))
      .then(res => {
        this.minimumPresentations = res.body.presentations || 0;
      });
  },
  methods: {
    deleteCourse: async function() {
      if (this.clicked == 0) {
        this.del = "Kurs löschen bestätigen";
      } else {
        ...
        router.push("/" + encodeURIComponent(this.account));
      }
      this.clicked++;
    },
    ...
    saveSettings: async function() {
      var res = await api.PUT(
        "/perc/" + encodeURI(this.account) + "/" + encodeURI(this.course),
        {
          type: this.settings.minimumType,
          minimum:
            this.settings.minimumType == "percent"
              ? parseFloat(this.settings.minimumPercent)
              : parseInt(this.settings.minimumPoints)
        }
      );
      ...
      var res = await api.PUT(
        "/pres/" + encodeURI(this.account) + "/" + encodeURI(this.course),
        { presentations: parseInt(this.settings.minimumPresentations) }
      );
